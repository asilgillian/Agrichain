...
-   **Procurement Field Capture (Mobile):** New `procurement` tab with batch list, contribution capture (farmer search + weight), batch lock, and station handover that creates a delivery and routes the agent to the delivery view; downstream weight/QC submission and farmer payment are gated by per-action permissions and are surfaced inline as the delivery moves through the workflow.
-   **Pay-Farmer Flow:** Web (delivery detail) and mobile (delivery view) expose a Pay-Farmer dialog when a delivery is approved. Cash payments deduct the agent's assigned cash float in a single transaction and write a `DEDUCTION` row alongside the payment; mobile-money payments validate the recipient MSISDN/provider (MTN MoMo / Airtel Money) and are recorded as `pending_external` until the gateway is wired up (gated by the `MOMO_ENABLED` feature flag). New permissions `payments.disburse.cash` and `payments.disburse.momo` are enforced per method on top of `payments.write`.
